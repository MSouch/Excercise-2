...
import medallionImage from '../assets/APLS-Medallion-2025_Risk.png'

const {FiShield,FiDownload,FiHome,FiTarget}=FiIcons

// ============================================================================
// RISK NAVIGATOR CONFIGURATION
// ============================================================================

const SIMULATION_CONFIG = {
  name: 'Risk Navigator',
  credentialTitle: 'Certified Risk Navigator Expert',
  programName: 'Risk Management Navigator Program',
  completionCode: 'RN0300',
  ...
  successStatistics: {
    intro: 'Organizations implementing integrated risk management approaches like yours report:',
    metrics: [
      {
        metric: '47%',
        label: 'Reduction in unplanned maintenance events',
        direction: 'down'
      },
      {
        metric: '34%',
        label: 'Decrease in project schedule delays',
        direction: 'down'
      },
      {
        metric: '52%',
        label: 'Improvement in safety performance',
        direction: 'up'
      },
      {
        metric: '$4.2M',
        label: 'Average annual savings for similar facilities',
        direction: 'up'
      }
    ],
    overallSuccess: [
      {
        metric: '31%',
        label: 'Overall incident reduction',
        direction: 'down'
      },
      {
        metric: '27%',
      ...
      {
        metric: '+18 pt',
        label: 'Higher stakeholder satisfaction',
        direction: 'up'
      }
    ]
  },
  ...
  nextSteps: {
    immediateActions: [
      'Apply systematic risk assessment to your next work packages',
      'Implement feedback collection systems for continuous improvement',
      'Share risk integration techniques with your planning team',
      'Begin building organizational risk management capability'
    ],
    continueLinks: [
      { text: 'Pursue advanced risk management certifications', href: 'https://www.ap-networks.com/' },
      { text: 'Lead organizational risk management improvement initiatives', href: 'https://www.ap-networks.com/' },
      { text: 'Mentor other planners in risk integration techniques', href: 'https://www.ap-networks.com/' },
      { text: 'Explore other Navigator Series training courses', href: 'https://www.ap-networks.com/' }
    ]
  }
}
...
const Conclusion=()=> {
  ...
  useEffect(()=> {
    ...
    if (completed===total && !credentialId) {
      const stored=localStorage.getItem('riskNavigatorCredential')
      if (stored) {
        try {
      ...
      try {
        localStorage.setItem('riskNavigatorCredential',JSON.stringify(payload))
      } catch (e) {
  ...
  const allChallengesCompleted=(progress?.completedChallenges || 0)===(progress?.totalChallenges || 4)
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {SIMULATION_CONFIG.successStatistics.overallSuccess.map((item,idx)=> {
                const IconComp=item.direction==='down' ? FaArrowDown : FaArrowUp
                const color=item.direction==='down' ? 'text-success-600' : 'text-primary-600'
                const bg=item.direction==='down' ? 'bg-success-100' : 'bg-primary-100'
                return (
                  <motion.div
                    key={idx}
                    className="text-center"
                  ...
                  >
                    ...
                    <p className={`text-base font-semibold ${color} mb-1`}>{item.metric}</p>
              ...
              <div className="space-y-3">
                ...
                <ul className="space-y-2 text-gray-600">
                  {SIMULATION_CONFIG.nextSteps.continueLinks.map((link,idx)=> (
                    <li key={idx}>• <a href={link.href} className="text-primary-600 hover:underline" target="_blank" rel="noreferrer">{link.text}</a></li>
                  ))}
                </ul>
              </div>
            </div>
