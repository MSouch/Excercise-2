...
const AuthGuard = ({ children }) => {
  ...
  if (location.pathname === '/conclusion') {
    // Read the freshest progress from localStorage to avoid stale hook instance
    let effectiveCompleted = progress?.completedChallenges ?? 0
    try {
      const stored = localStorage.getItem('riskNavigatorProgress')
      if (stored) {
      ...
      console.warn('AuthGuard: Failed reading localStorage progress, falling back to hook state', e)
    }
