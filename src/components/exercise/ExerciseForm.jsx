...
const ExerciseForm=({exercise,onComplete})=> {
  ...
  useEffect(()=> {
    // Auto-save every 30 seconds (disabled on Model Answer view)
    if (showModelAnswer) return
  ...
  const handleAutoSave=()=> {
    if (!user || loading || showModelAnswer) return
  ...
  const handleSubmit=async (e)=> {
    ...
    try {
      ...
      setCompleted(true)
      // Learner will choose when to continue to model answer
    } catch (error) {
        ...
        <div className="p-6">
          ...
          {completed && !showModelAnswer ? (
            ...
            >
              ...
              <p className="text-gray-600 mb-6">Great work on completing this hands-on exercise. Proceed to view the model answer when ready.</p>
              <button
